...
// Function to handle alias operations
void handle_alias(const char *arguments) {
    char *aliases[100];  // You can adjust the array size as needed
    int alias_count = 0;

    // Tokenize the arguments into aliases
    ...
    // If no aliases are provided, list all aliases
    if (alias_count == 0) {
        for (int i = 0; i < alias_count; i++) {
            char *value = getenv(aliases[i]);
            if (value != NULL) {
                printf("%s='%s'\n", aliases[i], value);
            }
        }
